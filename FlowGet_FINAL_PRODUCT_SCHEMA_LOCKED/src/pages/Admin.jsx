...
function Products({setErr}){
  ...
  const save=async e=>{e.preventDefault();setErr('');setSaving(true);try{
    ...
    // Keep the canonical product write independent from optional/legacy columns.
    // Short Description is stored inside specifications.short_description.
    // It is deliberately NOT sent as a separate products column, so this save
    // remains compatible with older FlowGet databases too.
    const specifications=cleanSpecText?{details:cleanSpecText}:{ };
    if(shortDescription)specifications.short_description=shortDescription;
    const payload={
    ...
    // If an older database is missing an optional product column, remove only
    // that column and retry. This keeps legacy FlowGet databases compatible.
    ...
    for(let attempt=0;attempt<8;attempt++){
      ...
      if(!r.error)break;
      const m=String(r.error.message||'').match(missingColumn);
      const missing=m?.[1]||m?.[2];
      if(!missing||!(missing in writePayload))break;
      console.warn(`FlowGet product compatibility: skipping missing column ${missing}`);
      const next={...writePayload};delete next[missing];writePayload=next;
    }
  ...
  return <><Editor title={editing?'Edit Product':'Add Product'} onSubmit={save} onCancel={reset} saving={saving} fields={<>
    ...
    <label className="check"><input type="checkbox" checked={form.active} onChange={e=>setForm(f=>({...f,active:e.target.checked}))}/> Active</label>{uploading&&<div className="notice">Uploading media… please wait.</div>}
      ...
      try{
        ...
        if(result?.archived){
          setErr(`Product archived because it is used in ${result.order_item_count||0} order item(s). Order history is preserved.`);
        }
        clearProductDraft();
        setEditing(null);
        await load();
