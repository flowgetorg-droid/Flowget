...
const emptyProduct={name:'',slug:'',description:'',category_id:'',brand:'',sku:'',price:'',discount_price:'',stock:'0',main_image:'',video_url:'',active:true};
...
const SPEC_NAMES=['Color','Size','Material','Weight','Dimensions','Battery','Capacity','Warranty','Model','Brand','Compatibility','Other'];
...
function Products({setErr}){
  ...
  const edit=p=>{
    ...
    setSpecs(Object.entries(p.specifications&&typeof p.specifications==='object'?p.specifications:{}).map(([name,value])=>({name,value:String(value??'')})));
  ...
  return <><Editor title={editing?'Edit Product':'Add Product'} onSubmit={save} onCancel={reset} saving={saving} fields={<>
    ...
    <div className="field"><label>Description</label><textarea rows="5" value={form.description||''} onChange={e=>setForm(f=>({...f,description:e.target.value}))}/></div>
    <div className="specbox"><div className="spechead"><div><label>Specifications / Product Details</label><small>প্রতিটি product-এর জন্য Color, Size, Material, Warranty ইত্যাদি যোগ করুন।</small></div><button type="button" className="btn smallbtn" onClick={()=>setSpecs(s=>[...s,{name:'Color',value:''}])}>+ Add specification</button></div>{specs.map((sp,i)=><div className="specrow" key={i}><select aria-label="Specification name" value={SPEC_NAMES.includes(sp.name)?sp.name:'Other'} onChange={e=>setSpecs(s=>s.map((x,j)=>j===i?{...x,name:e.target.value==='Other'?'':e.target.value}:x))}>{SPEC_NAMES.map(n=><option key={n} value={n}>{n}</option>)}</select>{SPEC_NAMES.includes(sp.name)?null:<input placeholder="Custom specification name" value={sp.name} onChange={e=>setSpecs(s=>s.map((x,j)=>j===i?{...x,name:e.target.value}:x))}/>}<input placeholder="Specification value" value={sp.value} onChange={e=>setSpecs(s=>s.map((x,j)=>j===i?{...x,value:e.target.value}:x))}/><button type="button" onClick={()=>setSpecs(s=>s.filter((_,j)=>j!==i))}>×</button></div>)}</div>
  ...
  </>}/><Table columns={['name','sku','price','stock','active']} data={data} onEdit={p=>edit(p)} onDelete={async p=>{
...
function Orders({setErr}){const[data,setData]=useState([]),[loading,setLoading]=useState(true),[busy,setBusy]=useState('');const load=async()=>{try{const{data,error}=await supabase.from('orders').select('*').order('created_at',{ascending:false}).limit(200);if(error)throw error;setData(data||[])}catch(e){setErr(e.message)}finally{setLoading(false)}};useEffect(()=>{load()},[]);const update=async(id,status)=>{setBusy(id);setErr('');try{if(status==='Cancelled'){const{error}=await supabase.rpc('cancel_order_secure',{p_order_uuid:id});if(error)throw error}else{const{error}=await supabase.rpc('update_order_status_secure',{p_order_uuid:id,p_status:status});if(error)throw error}await load()}catch(e){setErr(e.message)}finally{setBusy('')}};if(loading)return <div className="card"><p>Loading orders…</p></div>;return <div className="tablewrap"><table><thead><tr>{['order_id','customer_name','phone','total','status','created_at'].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{data.map(o=><tr key={o.id}><td>{o.order_id}</td><td>{o.customer_name}</td><td>{o.phone}</td><td>৳{o.total}</td><td><select value={o.status||'Pending'} disabled={busy===o.id} onChange={e=>update(o.id,e.target.value)}>{['Pending','Confirmed','Processing','Shipped','Delivered','Cancelled'].map(x=><option key={x}>{x}</option>)}</select></td><td>{new Date(o.created_at).toLocaleString()}</td></tr>)}</tbody></table></div>}
