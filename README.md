...
## Latest stability fixes (September 2026)
- Direct `/product/<slug-or-SKU>` links now resolve exact slugs case-insensitively and also normalized SKU links such as `/product/MCD311`.
- Home page no longer depends on one `Promise.all()` call: a banner/settings/category failure cannot hide the product catalog.
- Product detail related-product failures no longer turn a valid product page into "Product not found".
- Admin Products loading is independent of Categories loading.
- Product editing keeps the existing local draft and uses the separate `product_images` table for gallery data.
- `FlowGet_Product_Admin_Compatibility_Fix.sql` safely adds missing product columns/table pieces with `ADD COLUMN IF NOT EXISTS`; it does not drop, truncate, or delete existing business data.

### Required one-time SQL for an older/partially migrated database
Run **`FlowGet_Product_Admin_Compatibility_Fix.sql`** once in Supabase SQL Editor if Admin Product Edit reports a missing-column/schema-cache error. Do **not** rerun the full schema just for this.

### Cloudflare
Deploy the full project with build command `npm run build` and output directory `dist`. The existing `public/_redirects` keeps direct SPA routes working.
